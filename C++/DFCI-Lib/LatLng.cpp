...
LatLng::~LatLng() {
    if ( nullptr != piCachedLambertII ) {
        delete piCachedLambertII ;
        piCachedLambertII = nullptr ;
    }
}

LatLng::operator LambertII*()  {
    if ( piCachedLambertII != nullptr ) {
        return piCachedLambertII ;
    }
    
    double x;
    double y;

    double lambda_w = longitude * M_PI /180;
    double phi_w = latitude * M_PI /180;
    ...
    piCachedLambertII = new LambertII(x,y);
    

    return piCachedLambertII ;
}

LatLng::operator string() {
    if ( cachedDFCI.length() > 0 ) {
        return cachedDFCI ;
    }
    ...
    LambertII *lambertII = (LambertII*)*this;
